        ...
        <table class="table table-bordered">
            <tbody>
                ...
                <tr>
                    <td>收货地址</td>
                    <td colspan="3">
                        {{ $order->address['address'] }}&nbsp;&nbsp;{{ $order->address['zip'] }}&nbsp;&nbsp;{{ $order->address['contact_name'] }}&nbsp;&nbsp;{{ $order->address['contact_tel'] }}
                    </td>
                </tr>
            <tr>
            ...
            @if($order->ship_status === \App\Models\Order::SHIP_STATUS_PENDING)
                <!-- 加上这个判断条件 -->
                @if($order->refund_status !== \App\Models\Order::REFUND_STATUS_SUCCESS && ($order->type !== \App\Models\Order::TYPE_CROWDFUNDING || $order->orderItems[0]->product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_SUCCESS))
                <tr>
                    <td colspan="4">
